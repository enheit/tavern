...
CI (run 28921479675, sha e1c870a): bundle ✓ ×3 (ubuntu/windows/macos) — **S0.2 DoD met**; web-test ✓; rust-test ✓ macos/windows. rust-test ubuntu FAILED on the coverage gate: adding the src-tauri shell (0%-covered GUI boilerplate) dropped workspace lines to 62.50%, tripping `--fail-under-lines 70` — but PLAN says coverage enforcement flips on at S2.1. Fixed in follow-up commit (llvm-cov → measure-only until S2.1); re-verified green (run recorded at S0.3 baseline).
